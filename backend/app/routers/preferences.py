...
from app.schemas.user import RenterPreferencesOut, LandlordPreferencesOut
...
def save_renter_prefs(prefs: dict, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    updated = set_renter_preferences(db, current_user.id, prefs)
    return updated
...
def save_landlord_prefs(prefs: dict, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    updated = set_landlord_preferences(db, current_user.id, prefs)
    return updated
